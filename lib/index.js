...
exports.deploy = async function ({ src, target, branch, remote, template, title, dryRun, history, betterTarget }) {
  ...
  const dirs = (await readdir(tmpDir)).filter(dir => dir.indexOf('.') !== 0 && dir !== 'index.html').sort((a, b) => {
    ...
    const aPrefix = extractPrefix(a);
    const bPrefix = extractPrefix(b);
...
exports.debug = async function ({ src, target, branch, remote, template, title, dryRun, history, betterTarget }) {
  const dirs = (await readdir("./tmp")).filter(dir => dir.indexOf('.') !== 0 && dir !== 'index.html').sort((a, b) => {
    const extractVersion = (str) => {
      const match = str.match(/(\d+\.\d+(\.\d+)?)/);
      return match ? match[0] : '0.0.0';
    };
